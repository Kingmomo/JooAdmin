...
public class PesananActivity extends AppCompatActivity {
    ...
    private void starter() {
        Query query = FirebaseFirestore.getInstance().collection("transaksi").orderBy("waktu", Query.Direction.ASCENDING);
    ...
    private void sort(String data) {
        ...
        if (data.equals("1")) {
            query = FirebaseFirestore.getInstance().collection("transaksi").whereEqualTo("status", "Konfirmasi Pembayaran Di Proses").orderBy("waktu", Query.Direction.ASCENDING);
        } else if (data.equals("0")) {
            query = FirebaseFirestore.getInstance().collection("transaksi").whereEqualTo("status", "Belum Di Konfirmasi").orderBy("waktu", Query.Direction.ASCENDING);
        } else if (data.equals("2")) {
            query = FirebaseFirestore.getInstance().collection("transaksi").whereEqualTo("status", "Selesai").orderBy("waktu", Query.Direction.ASCENDING);
        } else {
            query = FirebaseFirestore.getInstance().collection("transaksi").whereEqualTo("status", "Pesanan Di Proses").orderBy("waktu", Query.Direction.ASCENDING);
